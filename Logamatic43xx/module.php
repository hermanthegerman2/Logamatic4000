    ...
    public function ForwardData($JSONString)
    {
        ...
        switch ($datentyp) {
            case 'b0':
                ...
                IPS_LogMessage('Buderus Logamatic', $offset ." : ". chr(ord(substr($offset,0,1))) . chr(ord(substr($offset,1,1))));
                switch ($offset) {
                    case '00':
                        ...
                        break;
                    default:
                        $data = utf8_encode(chr(Command::Datenblock) . chr($this->ReadPropertyInteger('Bus')) . substr($stream, 2, 1) . ltrim(chr(substr($offset,0,1)),"0") . ltrim(chr(substr($offset,1,1)),"0")); // Rückantwort anfragen
                }
                $this->SendDataToParent(json_encode(Array("DataID" => "{0D923A14-D3B4-4F44-A4AB-D2B534693C35}", "Buffer" => $data)));
                sleep (0.2);
                $this->SwitchNM();
