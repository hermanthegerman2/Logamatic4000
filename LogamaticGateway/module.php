...
class LogamaticGateway extends IPSModule
{
    ...
    public function RequestErrorLog()
    {
        ...
        $this->SendDataToParent($data);
        $data = chr(Command::Datenblock).chr(Command::NUL).chr(0x01).chr(Command::NUL).chr(Command::Fehlerprotokoll).chr(Command::NUL).chr(Command::NUL).chr(Command::NUL).chr(Command::NUL).chr(Command::NUL);
        $this->SendDataToParent($data);
        return true;
    }
